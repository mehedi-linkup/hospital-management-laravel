					...
					<tbody v-for="(purchaseReturn,sl) in purchaseReturns" :key='sl'>
							<tr>
								...
								<td style="text-align:center;">
									<a title="Purchase Invoice" v-bind:href="'/purchase_return_inventory_invoice/'+ purchaseReturn.id" target="_blank"><i class="fa fa-file-text"></i></a>
									<span v-if="role != 'User'">
									<a  title="Edit" v-bind:href="'/purchase_return_inventory/'+ purchaseReturn.id"><i class="fa fa-edit"></i></a>
									<a href="" title="Delete Purchase" @click.prevent="deletePurchase(purchaseReturn.id)"><i class="fa fa-trash"></i></a>
									</span>
								</td>
							</tr>
    ...
    data(){
		...
		methods: {
			...
			getPurchaseReturnRecord(){
				...
				let url = '/get_purchase_return_inventory';
			...
			getPurchaseDetails(){
				...
				axios.post('/get_purchase_return_inventory_details', filter)
				.then(res => {
					this.purchaseReturns = res.data;
				})
				.catch(error => {
        ...
        deletePurchase(purchaseReturnId){
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.post('/purchase-return-inventory-delete',{id: purchaseReturnId}).then(res=>{
                        let r = res.data;
                        Swal.fire({
                            icon: 'success',
                            title: r.message,
