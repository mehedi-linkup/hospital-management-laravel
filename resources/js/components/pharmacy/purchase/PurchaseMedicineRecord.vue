					...
					<tbody v-for="(purchase,sl) in purchases" :key='sl'>
							<tr>
								...
								<td style="text-align:center;">
									<a title="Purchase Invoice" v-bind:href="'/purchase_medicine_invoice_print/'+ purchase.id" target="_blank"><i class="fa fa-file-text"></i></a>
									<span v-if="role != 'User'">
    ...
    data(){
		...
		methods: {
			checkReturnAndEdit(purchase){
				axios.get('/check_purchase_medicine_return/' + purchase.invoice_number).then(res=>{
					if(res.data.found){
						alert('Unable to edit. Purchase return found!');
					}else{
						location.replace('/purchase_medicine/'+purchase.id);
					}
				})
			},
			onChangeSearchType(){
			...
			getPurchaseDetails(){
				...
				axios.post('/get_purchase_medicine_details', filter)
				.then(res => {
					this.purchases = res.data;
				})
				.catch(error => {
        ...
        deletePurchase(purchaseId){
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.post('/delete-purchase-medicine',{id: purchaseId}).then(res=>{
                        let r = res.data;
                        Swal.fire({
                            icon: 'success',
                            title: r.message,
